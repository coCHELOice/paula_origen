    ...
    <section id="el-espacio" className="py-20 md:py-32 lg:py-40 bg-brand-bg overflow-hidden">
      ...
      <div className="px-5 sm:px-8 md:px-[8vw] mb-10 md:mb-16 max-w-6xl mx-auto flex flex-col md:flex-row md:items-end md:justify-between gap-4 md:gap-12">
        ...
        <p className="font-sans text-sm sm:text-base text-brand-muted font-light md:text-right md:max-w-[260px] shrink-0">
          Desliza para explorar el lugar donde vivirás la experiencia.
        </p>
      </div>

      {/* Horizontal scroll strip */}
      <div className="flex gap-3 sm:gap-4 overflow-x-auto no-scrollbar snap-x snap-mandatory pl-5 sm:pl-8 md:pl-[8vw]">
        {images.map((img, idx) => (
          <div
            key={idx}
            className="snap-start shrink-0 w-[72vw] sm:w-[52vw] md:w-[36vw] lg:w-[26vw] xl:w-[22vw] relative overflow-hidden group"
          >
        ...
        {/* End spacer */}
        <div className="shrink-0 w-5 sm:w-8 md:w-[8vw]" />
      </div>
    </section>
  );
}
