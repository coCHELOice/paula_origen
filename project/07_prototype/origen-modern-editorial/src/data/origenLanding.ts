export const content = {
  hero: {
    eyebrow: "RETIRO ORIGEN",
    title: "ORIGEN — Cinco días en el Valle del Elqui para pausar, respirar y reconectar.",
    subtitle: "Una experiencia íntima para mujeres que necesitan una pausa real. Movimiento, meditación, ritual y comunidad en el corazón del Valle del Elqui.",
    meta: ["Valle del Elqui · Alcohuaz", "16–20 de abril"],
    ctaPrimary: "Reserva tu lugar",
    ctaSecondary: "Ver programa"
  },
  queEsOrigen: {
    title: "Qué es ORIGEN",
    paragraphs: [
      "Cinco días para salir del ruido y habitar el cuerpo en un entorno natural cuidadosamente curado.",
      "Una experiencia que integra movimiento, rituales y comunidad para crear una pausa real de escucha y renovación profunda."
    ]
  },
  paraQuienEs: {
    title: "Para quién es ORIGEN",
    paragraphs: [
      "Para mujeres que sostienen demasiado y necesitan recuperar claridad, descanso y ritmo propio.",
      "Si buscas belleza consciente, naturaleza viva y una comunidad que acompañe sin artificio, ORIGEN puede ser para ti."
    ]
  },
  laExperiencia: {
    title: "La experiencia",
    intro: "ORIGEN se sostiene sobre cinco pilares pensados para facilitar una pausa real. Nada está planteado desde la exigencia; todo está dispuesto para que puedas descansar, reconectar y habitar la experiencia a tu propio ritmo.",
    pillars: [
      {
        name: "Anclaje corporal y meditación:",
        desc: "prácticas diarias para soltar tensión, aquietar el ruido y ayudar al cuerpo a recuperar su capacidad de descanso."
      },
      {
        name: "Ritual de temazcal:",
        desc: "un espacio de calor, pausa e introspección enfocado en la limpieza y la renovación."
      },
      {
        name: "Belleza consciente:",
        desc: "una experiencia guiada de slow beauty donde el cuidado de la piel se convierte en una forma de presencia."
      },
      {
        name: "Alimentación nutritiva:",
        desc: "una propuesta cuidadosamente diseñada para nutrir el cuerpo, sostener la energía y disfrutar del sabor."
      },
      {
        name: "Comunidad:",
        desc: "espacios naturales de conversación entre mujeres que comparten el valor de esta pausa."
      }
    ]
  },
  elLugar: {
    title: "El lugar",
    paragraphs: [
      "Nos reuniremos en Alcohuaz, en el corazón del Valle del Elqui: un territorio de contrastes, cielos despejados y una calma que invita naturally a bajar el ritmo.",
      "El entorno no es solo un telón de fondo, sino una parte esencial de la experiencia. Desde la mesa compartida hasta las noches bajo las estrellas, el lugar ofrece la distancia y la intimidad necesarias para dejar atrás las exigencias cotidianas y volver a lo esencial."
    ]
  },
  programa: {
    title: "Programa",
    intro: "El recorrido está pensado para acompañarte desde la llegada al cuerpo hasta una integración más profunda, sin prisa ni sobrecarga.",
    days: [
      {
        title: "Día 1 — Llegar y aterrizar",
        desc: "Bienvenida, apertura de la experiencia y cena para aterrizar el ritmo."
      },
      {
        title: "Día 2 — Enraizar",
        desc: "Silencio, meditación y temazcal. Prácticas de movilidad y slow beauty bajo las estrellas."
      },
      {
        title: "Día 3 — Abrir espacio",
        desc: "Movimiento consciente y respiración. Visita a Viña Alcohuaz y espacios de conversación íntima."
      },
      {
        title: "Día 4 — Integrar",
        desc: "Integración y descanso. Prácticas suaves, pausa y viaje sonoro."
      },
      {
        title: "Día 5 — Cerrar y volver",
        desc: "Cierre y despedida sin prisa para regresar con la experiencia ya integrada."
      }
    ]
  },
  paulaEquipo: {
    title: "Paula + equipo",
    intro: "ORIGEN es sostenido por una red de mujeres que aportan presencia, experiencia y cuidado a cada momento del retiro.",
    members: [
      {
        name: "Paula Suryaflow:",
        desc: "Con 25 años de trayectoria, guía procesos de meditación y consciencia corporal desde la presencia profunda."
      },
      {
        name: "Paula Pesse:",
        desc: "Chef especializada en alimentación consciente; conecta los sabores del territorio con el bienestar del cuerpo."
      },
      {
        name: "Paz Urra:",
        desc: "Especialista en Slow Beauty; entiende el cuidado de la piel como una forma de presencia y conexión."
      }
    ]
  },
  reserva: {
    title: "Reserva",
    intro: "Elige la alternativa que mejor responda a tu momento actual. Ambas opciones incluyen alojamiento, alimentación completa y acceso a todas las prácticas del retiro.",
    options: [
      {
        title: "Habitación compartida",
        price: "$1.590.000 CLP",
        desc: "Comparte esta experiencia con otras mujeres que vienen a vivir la misma pausa."
      },
      {
        title: "Habitación suite",
        price: "$2.590.000 CLP",
        desc: "Un espacio exclusivamente para ti, con mayor privacidad y ritmo propio."
      }
    ],
    ctaPrimary: "Reserva tu lugar"
  },
  faqCierre: {
    title: "Preguntas frecuentes",
    faqs: [
      {
        q: "¿Necesito experiencia previa en yoga o meditación?",
        a: "No. Todas las prácticas están pensadas para ser seguras y adaptables a distintos niveles de experiencia."
      },
      {
        q: "¿Qué incluye el retiro?",
        a: "Alojamiento, alimentación completa y acceso a todas las prácticas contempladas en la experiencia."
      },
      {
        q: "¿Qué pasa si tengo restricciones alimentarias?",
        a: "El menú puede adaptarse. Al confirmar tu lugar, podremos recoger esa información con anticipación."
      },
      {
        q: "¿Cómo llegar a Alcohuaz?",
        a: "Los detalles logísticos de llegada se entregarán al momento de la reserva o postulación."
      },
      {
        q: "¿Cómo reservo mi lugar?",
        a: "Puedes iniciar tu reserva a través del botón principal de la página y seguir las indicaciones del proceso."
      }
    ],
    closing: "Una pausa real para soltar, reconectar y volver a ti.",
    ctaFinal: "Reserva tu lugar"
  }
};
